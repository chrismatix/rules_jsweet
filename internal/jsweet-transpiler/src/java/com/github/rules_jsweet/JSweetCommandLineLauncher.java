...
/**
 * For the purpose of customizing cli options, copied from:
 https://github.com/cincheo/jsweet/blob/master/transpiler/src/main/java/org/jsweet/JSweetCommandLineLauncher.java
 */
public class JSweetCommandLineLauncher {
	...
	private static JSAP defineArgs() throws JSAPException {
		...
		// Base directory
		optionArg = new FlaggedOption("baseDir");
		optionArg.setLongFlag("baseDir");
		optionArg.setHelp(
				"The directory JSweet uses to store temporary files such as extracted candies. JSweet uses '.jsweet' if left unspecified.");
		optionArg.setStringParser(FileStringParser.getParser());
		optionArg.setRequired(false);
	...
	private static class JSweetTranspilationTask implements TranspilationTask {
		...
		public void run() throws Exception {
			...
			try {
				...
				try (JSweetTranspiler transpiler = new JSweetTranspiler(jsapArgs.getFile("baseDir"), null, factory, jsapArgs.getFile("workingDir"),
